...
class Nozin {
    ...
    public Nozin(Personagem elemento, Nozin esq, Nozin dir, int nivel) {
    ...
    public static int getNivel(Nozin no) {
...
class AVL {
    ...
    private Nozin balancear(Nozin no) throws Exception {
        if (no != null) {
            int fator = Nozin.getNivel(no.dir) - Nozin.getNivel(no.esq);

            if (Math.abs(fator) <= 1) {
                no.setNivel();

            } else if (fator == 2) {
                int fatorFilhoDir = Nozin.getNivel(no.dir.dir) - Nozin.getNivel(no.dir.esq);
                ...
                int fatorFilhoEsq = Nozin.getNivel(no.esq.dir) - Nozin.getNivel(no.esq.esq);
    ...
    private Nozin rotacionarDir(Nozin no) {
        Nozin noEsq = no.esq;
        Nozin noEsqDir = noEsq.dir;
    ...
    private Nozin rotacionarEsq(Nozin no) {
        Nozin noDir = no.dir;
        Nozin noDirEsq = noDir.esq;
